...
local volume = {device = '', display_notification = false, notification = nil, delta = 2}
...
local function parse_output(stdout)
    ...
    if stdout:find("%[off%]") then
        volume_icon_name="notification-audio-volume-muted"
        return level.."% <span color=\"red\"><b>Mute</b></span>"
    end
    level = tonumber(string.format("% 3d", level))

    if (level >= 0 and level < 25) then
        volume_icon_name="notification-audio-volume-low"
    elseif (level < 50) then
        volume_icon_name="notification-audio-volume-low"
    elseif (level < 75) then
        volume_icon_name="notification-audio-volume-medium"
    else
        volume_icon_name="notification-audio-volume-high"
    end
    return level.."%"
end
...
--{{{ Widget creation
    volume.widget = wibox.widget {
        {
            id = "icon",
            image = PATH_TO_ICONS .. "notification-audio-volume-muted.svg",
            resize = true,
            widget = wibox.widget.imagebox,
        },
        layout = wibox.container.margin(1, 1, 1, 1),

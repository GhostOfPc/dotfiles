...
local function worker(args)
    ...
    -- Settings
    local ICON_DIR      = "/usr/share/icons/Papirus/48x48/status/"
    ...
    net_icon:set_image(ICON_DIR.."notification-network-wireless.svg")
	...
	awful.spawn.easy_async("awk 'NR==3 {printf \"%3.0f\" ,($3/70)*100}' /proc/net/wireless", function(stdout, stderr, reason, exit_code)
        ...
        if signal_level == nil then
            connected = false
            net_text:set_text(" N/A ")
            net_icon:set_image(ICON_DIR.."notification-network-wireless-disconnected.svg")
        else
            connected = true
            net_text:set_text(string.format("%"..indent.."d%%", signal_level))
            if signal_level < 25 then
                net_icon:set_image(ICON_DIR.."nm-signal-25.svg")
            elseif signal_level < 50 then
                net_icon:set_image(ICON_DIR.."nm-signal-50.svg")
            elseif signal_level < 75 then
                net_icon:set_image(ICON_DIR.."nm-signal-75.svg")
            else
                net_icon:set_image(ICON_DIR.."nm-signal-100.svg")
            end
        end
    end
